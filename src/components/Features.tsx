      ...
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        <Reveal>
          <div className="flex items-baseline gap-5">
            <span className="font-serif text-xl text-muted">03</span>
          ...
          {features.map((f, i) => (
            <Reveal key={f.title} delay={i * 80}>
              <div className="group grid grid-cols-12 items-baseline gap-x-4 gap-y-3 border-b border-ink/15 py-7 transition-colors duration-300 hover:bg-ink/[0.03] md:py-9">
                <div className="col-span-2 font-serif text-base text-muted md:col-span-1 md:text-lg">
                  {pad(i + 1)}
                </div>
                <h3 className="col-span-10 font-serif text-2xl tracking-tight transition-all duration-300 group-hover:translate-x-1 group-hover:scale-[1.02] origin-left md:col-span-4 md:text-3xl">
                  {f.title}
                </h3>
